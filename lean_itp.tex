...
\mdfdefinestyle{mdgoalbox}{
    style=mdremarkbox, % inherit from remark
    linecolor=MainColor,
...
\declaretheoremstyle[
    ...
    bodyfont=\normalfont\small,
    mdframed={style=mdgoalbox},
    headpunct={\\[3pt]},
    postheadspace={0pt}
]{goalbox}
\declaretheorem[style=goalbox,name=Cele,numbered=no]{lesson-goals}
...
\maketitle
\begin{abstract}
    \noindent Niniejszy skrypt zawiera informacje dotyczące systemu Lean 4 oraz podstawy teoretyczne, które warto poznać, aby dowiedzieć się, na czym \textit{interactive theorem proving} w ogóle polega. Powstawał on przy okazji kursu, który miałem przyjemność przygotować i poprowadzić jako student czwartego roku na AGH w Krakowie.
\end{abstract}
\vspace*{\fill}
...
\begin{remark}[Plausible]
    Istnieje paczka Leana o nazwie \textbf{Plausible}, która pomaga w szybkim szukaniu kontrprzykładów do hipotez. Jej działania opiera się po prostu na losowym próbkowaniu argumentów i sprawdzaniu, czy teza zachodzi. Jeśli nie, to zwraca znaleziony kontrprzykład (przez informację diagnostyczną błędu kompilacji).
    \begin{minted}{lean4}
import Plausible
...
Jak na porządny język programowania przystało, Lean posiada operatory umożliwiające kontrolę kolejności ewaluacji wyrażeń. Operatory \texttt{<|} i \texttt{|>} zachowują się tak samo jak w F\# czy Elm, sam operator \texttt{|>} znajdziemy również w Elixirze czy OCamlu.
...
W ten sekcji zajmiemy wykazaniem kilku ciekawych faktów matematycznych.
Każdy z tych przykładów będzie okazją do zaznajomienia się z pewnym fragmentem Mathliba oraz poznania nowych konstrukcji składniowych i taktyk Leana.

\subsection{Zbiór z relacją równoważności}

\begin{lesson-goals}[czego się nauczymy?]
...
W Leanie istnieją \vocab{klasy typów} (ang. \textit{type classes}), które są trochę jak interfejsy w Javie czy C\#, ale z pewnymi różnicami. Między innymi: można dodawać instancję klasy typów do istniejącego typu bez jego modyfikacji, istnieją parametryzowane klasy typów, klasy typów mogą dziedziczyć po innych klasach typów, klasy typów mogą być instancjonowane automatycznie lub w wyniku dowodzenia.
...
\begin{lesson-goals}[czego się nauczymy?]
  Typy zależne, koercja typów, inferencja klasy typów, taktyki \Lean{change}, \Lean{unfold}, \Lean{field_simp}, \Lean{infer_instance}.
\end{lesson-goals}
...
Użyliśmy tutaj taktyki \Lean{unfold}, która \emph{rozwija} definicję \Lean{op} w danym miejscu.
...
Teraz pokażemy, że $(A, \star)$ jest monoidem, czyli ma element neutralny. W tym celu należy powiedzieć, że typ $A$ ma jedynkę (równą $2$), a następnie, że ta jedynka istotnie jest elementem neutralnym, tzn. $2 \star a = a \star 2 = a$ dla każdego $a \in A$.
\begin{minted}{lean4}
...
\subsection{Równanie funkcyjne}

\begin{lesson-goals}[czego się nauczymy?]
    Słowo kluczowe \Lean{obtain}, taktyki \Lean{specialize}, \Lean{funext}, \Lean{refine}, \Lean{simpa}.
\end{lesson-goals}

Zajmiemy się teraz pewnym zadaniem, które pojawiło się na shortliście Międzynarodowej Olimpiady Matematycznej w 2013 roku (\href{https://www.imo-official.org/problems/IMO2013SL.pdf#page=7}{zadanie N1}). Oryginalnie należało znaleźć wszystkie rozwiązania równania funkcyjnego, ale my ograniczymy się do pokazania, że podane rozwiązanie jest jedynym możliwym.

\begin{theorem}
    Funkcja $f(x) = x$ jest jedyną funkcją $f : \NN_+ \to \NN_+$ spełniającą
    \[ m^2 + f(n) \mid mf(m) + n \]
    dla wszystkich $m, n \in \NN_+$.
\end{theorem}
Jako że dowód może być trudniejszy, niż w poprzednich przykładach, najpierw przedstawimy go w języku naturalnym.
\begin{proof}
    Łatwo sprawdzić, że $f(x) = x$ spełnia warunek zadania. Pokażemy teraz, że jest to jedyne rozwiązanie.

    Podstawmy $m = n = 2$. Otrzymujemy
    \[ 4 + f(2) \mid 2f(2) + 2. \]
    Z tego wynika, że istnieje $k \in \NN_+$ takie, że
    \begin{align*}
        k(4 + f(2)) &= 2f(2) + 2 \\
        4k + kf(2) &= 2f(2) + 2 \\
        (k - 2)f(2) &= 2 - 4k \\
    ...
    \begin{align*}
        ...
        n(n - 1) &\leq f(n)(n - 1).
    \end{align*}
    Jeśli $n = 1$, to oczywiście $f(1) = 1$. Jeśli $n \neq 1$, to możemy podzielić przez $n - 1$ i otrzymujemy $n \leq f(n)$. Ostatecznie mamy $f(n) = n$ dla każdego $n \in \NN_+$.
\end{proof}
...
\end{minted}
Funkcja \Lean{id} to funkcja identycznościowa, to znaczy $\opname{id}(x) = x$ dla każdego $x$.

Chcemy pokazać równoważność, więc musimy udowodnić oba kierunki. Możemy więc użyć taktyki \Lean{constructor}, która tworzy dwa cele do udowodnienia (z czego jeden będzie bardzo prosty).
\begin{minted}{lean4}
...
Zacznijmy od pokazania, że $f(2) = 2$. Aby to zrobić, należy podstawić $m = n = 2$, które można oczywiście zrealizować za pomocą \Lean{have} oraz aplikacji, ale my użyjemy taktyki \Lean{specialize}.
\begin{minted}{lean4}
...
Teraz musimy skorzystać z faktu, że $a \mid b$ wtedy i tylko wtedy, gdy istnieje $c$ takie, że $b = ca$. W Leanie takie twierdzenie jest dostępne jako \href{https://leanprover-community.github.io/mathlib4_docs/Mathlib/Algebra/Divisibility/Basic.html#exists_eq_mul_left_of_dvd}{\Lean{exists_eq_mul_left_of_dvd}}.
Czytelnik powinien zwrócić uwagę na jego dokładne sformułowanie -- korzystamy z faktu, że $(\NN_+, \cdot)$ jest półgrupą przemienną.
\begin{minted}{lean4}
have f2_eq_2 : f 2 = 2 := by
  specialize h 2 2
  ...
  sorry
\end{minted}
Z założeniem, które zawiera kwantyfikator egzystencjalny, dosyć niewygodnie jest pracować. Wolelibyśmy mieć bezpośrednio zmienną i założenie o niej. Oby to osiągnąć, wystarczy zamienić \Lean{have} na \Lean{obtain}.
\begin{minted}{lean4}
have f2_eq_2 : f 2 = 2 := by
  specialize h 2 2
...
Teraz możemy już pokazać, że $k = 1$. Z takim celem powinna poradzić sobie już taktyka \Lean{nlinarith}, ale, jak Czytelnik raczy sprawdzić, sobie nie radzi\footnote{Przynajmniej w wersji Lean v26.0.0, w przyszłości może się to zmienić.}. Problemem jest to, że pracujemy na podtypie typu \Lean{ℕ}, a nie na samym \Lean{ℕ}. Zmieńmy to za pomocą \Lean{PNat.dvd_iff} oraz \Lean{PNat.eq}, kończąc tę część dowodu.
\begin{minted}{lean4}
import Mathlib.Data.PNat.Basic
import Mathlib.Tactic.Linarith
...
Aktualny cel to $f = \opname{id}$. Wolelibyśmy pracować z celem postaci $f(x) = x$ dla każdego $x$. Aby wykonać takie przejście, możemy użyć taktyki \Lean{funext}, która zamienia równość funkcji na równość wartości funkcji dla dowolnego argumentu oraz twierdzenia \Lean{id_eq}, które upraszcza $\opname{id}(x)$ do $x$.
\begin{minted}{lean4}
...
Teraz moglibyśmy kolejno udowodnić, że $f(x) \leq x$ oraz $f(x) \geq x$, następnie użyć \Lean{le_antisymm} ($f(x) \leq x \land f(x) \geq x \implies f(x) = x$), ale możemy od razu zasygnalizować do czego dążymy za pomocą \Lean{refine}, a Lean sam wygeneruje cele do udowodnienia.
\begin{minted}{lean4}
intro h
have f2_eq_2 : f 2 = 2 := sorry -- ...
funext x
...
Reszta dowodu jest już raczej prosta -- Czytelnik powinien spróbować go ukończyć samodzielnie (korzystając z twierdzenia \Lean{PNat.le_of_dvd} i taktyki \Lean{change} kiedy będzie to potrzebne). Cały dowód znajduje się poniżej.
\inputminted{lean4}{case_studies/FunctionalEquation.lean}
Taktyka \Lean{simpa} to połączenie \Lean{simp} oraz \Lean{assumption}, które domyślnie działa z hipotezą \Lean{this}. Zamiast \Lean{simpa} moglibyśmy więc napisać \Lean{simpa using this} albo \Lean{simp at this} i \Lean{assumption}.

\subsection{Kongruencja modulo}

\begin{lesson-goals}[czego się nauczymy?]
    Typy induktywne, taktyki \Lean{induction} i \Lean{gcongr}, środowisko \Lean{calc}.
\end{lesson-goals}
...
Typ induktywny to typ definiowany przez podanie jego konstruktorów. Dotychczas stworzyliśmy jeden typ, \Lean{Point}, który miał dokładnie jeden konstruktor, domyślnie nazwany \Lean{Point.mk}.
Korzystaliśmy już jednak z typów induktywnych z wieloma konstruktorami, np. \Lean{Or} z dwoma konstruktorami: \Lean{Or.inl} oraz \Lean{Or.inr}.
Również \Lean{Nat} jest typem induktywnym, który, zgodnie z aksjomatyką Peano, ma dwa konstruktory: \Lean{Nat.zero} (liczba $0$) oraz \Lean{Nat.succ} (następnik liczby naturalnej). Każdą liczbę naturalną można więc zbudować, zaczynając od zera i wielokrotnie stosując następnika. Poniżej znajduje się definicja liczb naturalnych w Leanie.
...
\begin{remark*}
    Składnia \Lean{structure} (której użyliśmy do zdefiniowania \Lean{Point}) jest w rzeczywistości skróconą formą składni \Lean{inductive}. Oprócz uproszczenia zapisu, struktury automatycznie generują pola dostępu (tzn. możemy użyć \Lean{point.x}). Zamiast
    \begin{minted}{lean4}
structure Point where
  x : Int
  y : Int
...
Wracając do konguencji, będziemy chcieli udowodnić poniższe twierdzenie przez indukcję względem $n$.
\begin{theorem}
    Dla każdej liczby naturalnej $n$ zachodzi
    \[ 4^n \equiv 1 + 3n \pmod{9}. \]
\end{theorem}
\begin{proof}
    Dla $n = 0$ mamy $4^0 = 1$ oraz $1 + 3 \cdot 0 = 1$, więc teza zachodzi. Załóżmy więc, że teza zachodzi dla pewnego $n$ i udowodnijmy ją dla $n + 1$ (to znaczy dla następnika $n$). Mamy
...
Aby przeprowadzić dowód przez indukcję, użyjemy taktyki \Lean{induction}.
\begin{minted}{lean4}
example {n : ℕ} : 4^n ≡ 1 + 3*n [MOD 9] := by
  ...
  | succ n ih => sorry
\end{minted}
W tym przypadku \Lean{ih} to nasza hipoteza indukcyjna (ang. \textit{induction hypothesis}). Teraz możemy przeprowadzić ciąg przekształceń za pomocą środowiska \Lean{calc}, które pozwala na pisanie łańcuchów relacji równoważności (takich jak równość czy kongruencja modulo), gdzie każdy krok jest uzasadniony dowodem.
\begin{minted}{lean4}
example {n : ℕ} : 4^n ≡ 1 + 3*n [MOD 9] := by
  induction n with
  | zero => rfl
...
Czytelnik powinien już być w stanie wypełnić luki samodzielnie, korzystając z twierdzeń dostępnych w \Lean{Mathlib.Data.Nat.ModEq} oraz taktyki \Lean{ring}. Cały dowód wygląda następująco.
\begin{minted}{lean4}
import Mathlib.Data.Nat.ModEq
import Mathlib.Tactic.Ring
...
Sformułowanie ostatniego kroku można uprościć, pisząc \Lean{_ = _ := by ring}, ponieważ ostateczny wynik jest naszym celem, więc Lean go zna. Poza tym, w niektórych krokach możemy użyć taktyki \Lean{gcongr}, która automatycznie stosuje pewne twierdzenia dotyczące kongruencji. Ostatecznie otrzymujemy:
\inputminted{lean4}{case_studies/CongruenceModulo.lean}

\begin{problem}
    Zrobić czwarty zestaw zadań:
    \inputminted{lean4}{problemsets/problemset04.lean}
\end{problem}

Wstęp teoretyczny do zadania 1 znajduje się w dodatku \ref{ssec:finite-integral-domain}.
...
\subsection{Pierścienie całkowite i ciała}
\label{ssec:finite-integral-domain}
...
Skończony pierścień całkowity można w Leanie opisać jako taki typ \Lean{R}, który spełnia \Lean{[CommRing R]}, \Lean{[IsDomain R]} i \Lean{[Finite R]}.
Klasa typów \Lean{CommRing} opisuje przemienny pierścień z jedynką\footnote{Pierścienie i półpierścienie w Leanie domyślnie mają jedynkę. Istnieją natomiast ich odpowiedniki bez jedynki, nazwane \texttt{NonUnitalRing} i \texttt{NonUnitalSemiring}.}, a \Lean{IsDomain} to własność półpierścienia z jedynką (\Lean{Semiring}), która oznacza nietrywialność i brak dzielników zera. Pierścień całkowity to więc po prostu \Lean{CommRing} z własnością \Lean{IsDomain}, a skończony pierścień całkowity to \Lean{CommRing} z własnościami \Lean{IsDomain} i \Lean{Finite}.

\subsection{Dalsza lektura}
...
\begin{itemize}
    \item taktyka \Lean{nth_rw},
    \item taktyka \Lean{by_cases},
    \item taktyka \Lean{congr},
    \item taktyka \Lean{contrapose!},
    \item silna indukcja (\Lean{induction n using Nat.strongRecOn}),
    \item (magiczne) taktyki \Lean{grind} i \Lean{aesop},
    \item taktyki z pytajnikiem (oprócz poznanego \Lean{simp?} mamy \Lean{exact?}, \Lean{apply?}, \Lean{easop?}, itp.),
    \item \Lean{structure} ... \Lean{deriving} ...,
    \item komenda \Lean{print axioms} (raczej w ramach ciekawostki).
\end{itemize}
