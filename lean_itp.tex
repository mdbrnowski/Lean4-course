...
Typ induktywny to typ definiowany przez podanie jego konstruktorów. Dotychczas stworzyliśmy jeden typ, \Lean{Point}, który miał dokładnie jeden konstruktor, domyślnie nazwany \Lean{Point.mk}. Korzystaliśmy już jednak z typów induktywnych z wieloma konstruktorami, np. \Lean{Or} z dwoma konstruktorami: \Lean{Or.inl} oraz \Lean{Or.inr}. Również \Lean{Nat} jest typem induktywnym, który, zgodnie z aksjomatyką Peano, ma dwa konstruktory: \Lean{Nat.zero} (liczba $0$) oraz \Lean{Nat.succ} (następnik liczby naturalnej).
\begin{minted}{lean4}
inductive Nat where
  | zero : Nat
  | succ (n : Nat) : Nat
\end{minted}
Składnia \Lean{structure} (której użyliśmy do zdefiniowania \Lean{Point}) jest w rzeczywistości skróconą formą powyższej składni \Lean{inductive}.
