...
\begin{definition}[przestrzeń pseudometryczna]
    Niech $X$ będzie niepustym zbiorem, a $d : X \times X \to \RR$ funkcją, zwaną pseudometryką, spełniającą następujące warunki dla dowolnych $x, y, z \in X$:
    \begin{itemize}[nosep]
        \item $d(x, x) = 0$,
        \item $d(x, y) = d(y, x)$,
        \item $d(x, y) \leq d(x, z) + d(z, y)$.
    \end{itemize}
    Wówczas para $(X, d)$ jest przestrzenią pseudometryczną.
\end{definition}

\begin{definition}[przestrzeń metryczna]
    Przestrzeń pseudometryczna $(X, d)$ jest przestrzenią metryczną, jeśli dodatkowo spełniony jest warunek:
    \[ d(x, y) = 0 \iff x = y \]
...
\begin{theorem}
    Udowodnij, że para $(A, \star)$ jest grupą abelową, gdzie $A = \QQ \setminus \{1\}$, a działanie $\star$ jest zdefiniowane jako
    \[ a \star b = ab - a - b + 2. \]
\end{theorem}
...
#check (⟨0, by decide⟩ : A)
\end{minted}
Element podtypu jest automatycznie wyposażony w wiele przydatnych pól, w tym: wartość, twierdzenie o spełnianiu warunku, twierdzenia o równoważności równości na typach i równości na podtypach.
\todo[inline]{występuje tutaj ↑, o którym nic nie pisałem}
\begin{minted}{lean4}
...
\todo{Pojęcie klasy typów i instancji powinno być w poprzedniej sekcji.}
\begin{minted}{lean4}
...
Teraz pokażemy, że $(A, \star)$ jest monoidem, czyli ma element neutralny. W tym celu należy powiedzieć, że typ $A$ ma jedynkę (równą $2$), a następnie, że ta jedynka istotnie jest elementem nautralnym, tzn. $2a = a \land a = 2a$. Nie trzeba pokazywać
\begin{minted}{lean4}
